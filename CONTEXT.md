...
### C3 Mode
An editor mode entered when importing a C3 sprite font. In this mode the editor uses C3 semantics (uniform grid, row-major character set, per-character display widths). Horizontal cell alignment is fixed to left to match C3's cell drawing and is hidden from the UI. Vertical alignment is reused to control how appended characters are distributed vertically: it computes the maximum visible glyph height among all appended characters and sets each appended character's top margin so the glyphs share a common visual baseline while the exported sprite sheet cells remain top-aligned (which is what the C3 runtime expects). `top` disables auto-distribution and lets users edit per-character top margins manually; `middle` centers each glyph within the maximum height; `bottom` aligns each glyph to the bottom of the maximum height. The default is `middle`. Cell margin defaults to 0 because C3 cells are contiguous. Character padding remains available because it offsets the glyph within the exported cell. Image margin and image padding default to 0 but remain editable so that a font located inside a larger sprite atlas can be shifted into place. The generic Upload Image button is hidden in C3 mode; the only way to change the base image is to import another C3 sprite font. If no custom font is uploaded, appended characters are rendered with a fallback system font.
