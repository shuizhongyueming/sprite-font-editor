...
export class CanvasSpace {
  ...
  get usableWidth(): number {
    const baseWidth =
      this.canvasWidth -
      this.imagePadding.left -
      this.imagePadding.right -
      this.imageMargin.left -
      this.imageMargin.right;
    if (!this.fontSpriteWidth) return baseWidth;
    return Math.min(
      baseWidth,
      this.fontSpriteWidth - this.imagePadding.left - this.imagePadding.right,
    );
  }
  ...
  get usableHeight(): number {
    const baseHeight =
      this.canvasHeight -
      this.imagePadding.top -
      this.imagePadding.bottom -
      this.imageMargin.bottom -
      this.imageMargin.top;
    if (!this.fontSpriteHeight) return baseHeight;
    return Math.min(
      baseHeight,
      this.fontSpriteHeight - this.imagePadding.top - this.imagePadding.bottom,
    );
  }
